...
let toastTimer, warnedRevision = null, demoTimer;
...
function renderConfig() {
  ...
  if (!networkDirty) {
    for (const [key,value] of Object.entries(show.network)) { const input=$('network-form').elements.namedItem(key); if(input) input.value=value; }
  }
...
function renderState() {
  $('output-badge').textContent=state.demo?'● DEMO · OUTPUT OFF':state.armed?'● OUTPUT LIVE':'● OUTPUT HELD';
  $('output-badge').classList.toggle('live',state.armed);
  $('footer-state').textContent=state.demo?'DEMO / NO MA OUTPUT':state.armed?'OUTPUT LIVE':'OUTPUT HELD';
  ...
  $('arm-output').disabled=state.armed||state.demo||state.ma.state!=='ready'||!state.input.running;
  $('hold-output').disabled=!state.armed;
  $('start-psn').disabled=state.input.running||state.demo;
  $('stop-psn').disabled=!state.input.running;
  $('connect-ma').disabled=state.ma.running||state.demo;
  ...
  for(const b of state.blocks) {
    ...
    card.querySelector('[data-status]').style.color=b.status==='Live'||b.status==='Preview'?'var(--accent)':'var(--warn)';
...
$('arm-output').addEventListener('click',()=>run(()=>action('arm')));
...
$('network-form').addEventListener('submit',e=>{e.preventDefault();run(async()=>{const draft=structuredClone(show);for(const key of Object.keys(draft.network)){const input=$('network-form').elements.namedItem(key);if(input)draft.network[key]=input.type==='number'?Number(input.value):input.value.trim();}await saveShow(draft);networkDirty=false;$('save-status').textContent='Saved on this device';toast('Network settings saved');});});
...
$('connect-ma').addEventListener('click',()=>run(async()=>{checkNetworkSaved();await action('connect_ma',{password:$('ma-password').value});$('ma-password').value='';}));
...
async function demoStart(){requireHeld();await action('demo',{enabled:true});toast('Demo signals enabled. MA output is disconnected.');view('blocks');}
...
$('demo-sliders').addEventListener('input',e=>{if(!e.target.dataset.demo)return;const id=Number(e.target.dataset.demo),level=Number(e.target.value);$('demo-value-'+id).value=level+'%';clearTimeout(demoTimer);demoTimer=setTimeout(()=>run(()=>api('action',{action:'demo_level',id,level})),50);});
(async()=>{const hash=new URLSearchParams(location.hash.slice(1));const key=hash.get('key');if(key){history.replaceState(null,'',location.pathname);try{await api('login',{key});}catch(e){$('login-error').textContent=e.message;}}try{await init();}catch(e){$('login').hidden=false;$('app').hidden=true;}setInterval(poll,200);})();
