...
$('login-form').addEventListener('submit',async e=>{e.preventDefault();try{await api('login',{key:$('access-key').value});$('access-key').value='';await init();}catch(error){$('login-error').textContent=error.message;}});
